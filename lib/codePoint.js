...
function codePointWidth(code) {
    ...
    if (code < 0x10000) {
        return code < 0xAC00 ? (
            ...
            code <= 0xFAFF
                ...
                ? ((code <= 0xD7A3 || code >= 0xF900) ? 2 : 1) // W [11,684]
                // Alphabetic Presentation Forms
                : code === 0xFB1E ? 0 : 1 // Mn [1]
        ) : code < 0xFE6C ? (
            code < 0xFE30 ? (
    ...
    // Mahjong Tiles ... CJK Unified Ideographs Extension G (1F000 - 3134F)
    if (code < 0x3FFFE) {
        return (code < 0x1F200 ? (
            // Mahjong Tiles
            code === 0x1F004 // W [1]
            ...
            || (code >= 0x1F90C && code <= 0x1F978 && code !== 0x1F93B && code !== 0x1F946) // W [107]
            || (code >= 0x1F97A && code !== 0x1F9CC) // W [133]
        ) : code < 0x1FAD7 ? (
            // Symbols and Pictographs Extended-A
            code >= 0x1FA70 && (code < 0x1FA90 ? (
                code <= 0x1FA74 // W [5]
                || (code >= 0x1FA78 && code <= 0x1FA7A) // W [3]
                || (code >= 0x1FA80 && code <= 0x1FA86) // W [7]
            ) : (
                code <= 0x1FAA8 // W [25]
                || (code >= 0x1FAB0 && code <= 0x1FAB6) // W [7]
                || (code >= 0x1FAC0 && code <= 0x1FAC2) // W [3]
                || code >= 0x1FAD0 // W [7]
            ))
        ) : (code >= 0x20000 && (
            // CJK Unified Ideographs Extension B ... CJK Compatibility Ideographs Supplement
            code <= 0x2FFFD // W [65,534]
            // CJK Unified Ideographs Extension G
            || code >= 0x30000 // W [65,534]
        ))) ? 2 : 1;
    }
