...
function wrapLine(string, columns, hard) {
    ...
    const words = string.trim().split(' ').map((word) => [word, stringWidth(word)]);
    // loop through each word
    words.forEach(([word, len], ix) => {
        // if the current row is not empty, a space must be added before this word
        if (rl > 0) rl += 1;
        ...
        if (hard && len > columns) {
            ...
            for (let regex = ansiRegex(), m = regex.exec(word); m; m = regex.exec(word)) {
                ...
                if (j > i) {
                    ...
                    for (const [char, w] of charWidths(word.slice(i, j))) {
            ...
            if (i < word.length) {
                ...
                // loop through the characters in this final chunk
                for (const [char, w] of charWidths(word.slice(i))) {
                    // check if a line break is needed
                    if (rl + w > columns) {
                        // finalize the current row
                        rows.push(row + closeAnsiStack(ansiStack, ax, ay));
        ...
        if (i < word.length) {
            ...
            if (ay < iy) {
                // add any new opening escapes to the current row (rl will always be > 0 here)
                row += ansiStack.filter(([,,, x, y]) => (x === ix && ay < y)).map(([s]) => s).join('');
                // update the active point in the ansi stack to the beginning of the current chunk
                ay = iy;
            }
            // add the final chunk to the current row
            row += word.slice(i);
        }
 ...
 * @param {boolean} [hard=false] - enable hard wrap mode
 * @returns {string}
 */
module.exports = function wordWrap(string, columns, hard = false) {
    return String(string)
        ...
        .split('\n')
        // wrap each line
        .map((line) => wrapLine(line, columns, hard))
