...
function parseEscape(ansiStack, seq, ix, iy) {
    // eslint-disable-next-line no-control-regex
    const { 1: code, 2: link } = seq.match(/[\u001B\u009B](?:\[(\d+)(?:;\d+)*m|\]8;;(.*)\u0007)/) || {};
    // `cx` & `cy` store the `x` & `y` index of the lowest item in the stack closed by this sequence
    let [cx, cy] = [Infinity, Infinity];
        ...
        if (hard && (len > columns || (!ltrim && ax < 0 && rl + len > columns && rl < columns))) {
            ...
            for (const [chunk, isEscape] of parseAnsi(word)) {
                ...
                if (isEscape) {
                    ...
                    const [cx, cy] = parseEscape(ansiStack, chunk, ix, iy);
                    if (ax < ix) {
