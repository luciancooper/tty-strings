...
function isZwjSequence(sequence) {
    ...
    for (i = j + 1; i < sequence.length; i = j + 1) {
        ...
        if (cp2 === undefined) {
            // if seq must be fully qualified, default emoji presentation is required, otherwise accept any `Emoji`
            if (!(fullyQualified ? (props1 & 2) : (props1 & 1))) return false;
        } else if (cp2 === 0xFE0F) {
...
module.exports = { emojiProps, isEmojiSequence };